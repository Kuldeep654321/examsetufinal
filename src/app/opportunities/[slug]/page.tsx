...
export default async function OpportunityDetailPage({ params }: PageProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-gradient-to-br from-emerald-900 to-teal-950 text-white rounded-3xl p-6 shadow-md space-y-2">
          <div className="flex items-center space-x-2 text-emerald-400 text-xs font-bold uppercase">
            <Award className="w-4 h-4" /> Financial Aid / Fellowship Benefit
          </div>
          <p className="text-2xl sm:text-3xl font-black text-white">
            {opp.financial_aid_amount || 'Fully Funded Merit Grant'}
          </p>
      ...
      {/* Official Guidelines Citation Box */}
      <div className="p-4 bg-slate-100 rounded-2xl text-xs text-slate-600 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <span className="flex items-center gap-1.5">
          <ShieldCheck className="w-4 h-4 text-emerald-600" /> Official Source Reference: {opp.org?.name}
        </span>
        <a
          href={opp.official_source_url}
          target="_blank"
          rel="noopener noreferrer"
          className="text-blue-600 hover:text-blue-700 font-bold flex items-center gap-1 self-start sm:self-auto"
        >
          View Scheme Guidelines PDF <ExternalLink className="w-3.5 h-3.5" />
        </a>
      </div>
    </div>
  );
